...
public class FactorialActivity extends AppCompatActivity {
    ...
    private void onButtonClick(String button) {
        ...
        String subStr = current.isEmpty() ? "" : current.substring(0, current.length() - 1);
        tvUserInput.setText(subStr + button + "!");
    }
    ...
    private void onDeleteClick() {
        ...
        tvUserInput.setText(current.substring(0, current.length() - 2) + "!");
    }

    private void onEqualsClick() {
        String current = tvUserInput.getText().toString();
        int result = handleFactorial(current);
